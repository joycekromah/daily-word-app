...
export default function Home() {
  ...
  useEffect(() => {
    const savedFromStorage = localStorage.getItem("savedWords");
    const knownFromStorage = localStorage.getItem("knownWords");

    if (savedFromStorage) setSaved(JSON.parse(savedFromStorage));
    if (knownFromStorage) setSaved(JSON.parse(knownFromStorage));
  ...
  useEffect(() => {
    localStorage.setItem("knownWords", JSON.stringify(saved));
  }, [saved]);
  ...
  const moveToKnown = (learntWord) => {
    ...
    if (!known.find(item => item.word === learntWord.word)) {
            ...
            <ul className="list-unstyled">
              {sortedSaved.map((entry, index) => (
                <li key={index} className="mb-2">
                  ...
                  <button className="move-btn" onClick={() => moveToKnown(entry)}>
                    Mark as known
                  </button>
                </li>
              ))}
